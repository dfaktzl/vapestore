/* ==========================================================================
   STOREFRONT MAIN CONTROLLER (AeroVape REBRAND)
   ========================================================================== */
...
class StoreApp {
  ...
  async init() {
    ...
    const savedLogo = localStorage.getItem("aero_vape_logo_style") || "2";
  ...
  setupAgeGate() {
    ...
    // Check age consent
    if (ageGate && localStorage.getItem("aero_vape_age_verified") === "true") {
      ageGate.classList.add("hidden");
    }

    if (verifyBtn) {
      verifyBtn.addEventListener("click", () => {
        localStorage.setItem("aero_vape_age_verified", "true");
    ...
    if (clearCookieBtn) {
      clearCookieBtn.addEventListener("click", (e) => {
        e.preventDefault();
        localStorage.removeItem("aero_vape_age_verified");
  ...
  loadCart() {
    const stored = localStorage.getItem("aero_vape_cart");
    if (stored) {
      try {
  ...
  saveCart() {
    localStorage.setItem("aero_vape_cart", JSON.stringify(this.cart));
    this.updateCartUI();
  }
  ...
  submitOrder() {
    ...
    const metadata = {
      ...
      ip: localStorage.getItem("aero_vape_visitor_ip") || "Unknown"
    };
    ...
    const storedOrders = localStorage.getItem("aero_vape_orders");
    let ordersList = [];
    if (storedOrders) {
      try {
    ...
    localStorage.setItem("aero_vape_orders", JSON.stringify(ordersList));
    ...
    if (successPayid) successPayid.innerText = bank.payId || "orders@aerovape.co";
    ...
    const targetEmail = settings.contactEmail || "orders@aerovape.co";
    ...
    .then(() => {
      ...
      // Clear cart
      this.cart = [];
      localStorage.removeItem("aero_vape_cart");
      this.updateCartUI();
      ...
      localStorage.setItem("aero_vape_latest_order", JSON.stringify(latestOrder));
  ...
  sendContactFormSubmit(name, email, message) {
    const targetEmail = (this.config && this.config.settings && this.config.settings.contactEmail) || "orders@aerovape.co";
  ...
  bindEvents() {
    ...
    // AeroVape Unique Features bindings
  ...
  generateSEOSchema() {
    ...
    const schemas = [];
    const siteUrl = "https://aerovape.co";
    ...
    const orgSchema = {
      ...
      "name": this.config.settings.siteName || "AeroVape",
      "url": siteUrl,
    ...
    if (this.guides && this.guides.length > 0) {
      this.guides.forEach(guide => {
        const articleSchema = {
          ...
          "author": {
            "@type": "Person",
            "name": guide.author || "AeroVape Education"
          },
          "publisher": {
            "@type": "Organization",
            "name": this.config.settings.siteName || "AeroVape",
  ...
  async trackVisitor() {
    try {
      ...
      let visitId = sessionStorage.getItem("aero_vape_visit_id");
      let visitLogged = sessionStorage.getItem("aero_vape_visit_logged");
      let visitorId = localStorage.getItem("aero_vape_visitor_id");

      if (!visitorId) {
        visitorId = "usr_" + Date.now() + "_" + Math.random().toString(36).substring(2, 11);
        localStorage.setItem("aero_vape_visitor_id", visitorId);
      }

      if (!visitId) {
        visitId = "vst_" + Date.now() + "_" + Math.random().toString(36).substring(2, 11);
        sessionStorage.setItem("aero_vape_visit_id", visitId);
      }
      ...
      if (visitLogged !== "true") {
        ...
        try {
          ...
          if (geoResponse.ok) {
            ...
            if (data && data.ip) {
              geoData = data;
              localStorage.setItem("aero_vape_visitor_ip", data.ip);
        ...
        if (response.ok) {
          sessionStorage.setItem("aero_vape_visit_logged", "true");
  ...
  applyLogoStyle(style) {
    ...
    let html = "";
    if (style === "1") {
      html = `<span style="font-family:var(--font-title); font-weight:400; font-size:20px; color:var(--text-primary); letter-spacing:4px; text-transform:lowercase;">a e r o v a p e</span>`;
    } else if (style === "3") {
      html = `<span style="font-family:monospace; font-weight:700; font-size:20px; color:var(--text-primary); letter-spacing:1px; text-transform:uppercase;">[ <span style="color:var(--primary-accent);">AERO:VAPE</span> ]</span>`;
    } else {
      // Default: Style 2 (Emblem)
      html = `<span style="font-family:var(--font-title); font-weight:800; font-size:20px; color:var(--text-primary); letter-spacing:0.5px; text-transform:uppercase;">🔵 aero<span style="color:var(--primary-accent);">Vape</span></span>`;
    }
    target.innerHTML = html;
    localStorage.setItem("aero_vape_logo_style", style);
