...
class AdminApp {
  ...
  async init() {
    ...
    if (sessionStorage.getItem("aero_vape_admin_authenticated") === "true") {
  ...
  bindLoginEvent() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (authenticated) {
        sessionStorage.setItem("aero_vape_admin_authenticated", "true");
        document.getElementById("admin-login-overlay").style.display = "none";
        await this.initDashboard();
      } else {
  ...
  async loadOrders() {
    const stored = localStorage.getItem("aero_vape_orders");
    if (stored) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data) {
            ...
            this.orders = list;
            localStorage.setItem("aero_vape_orders", JSON.stringify(list));
          } else {
            this.orders = [];
          }
        }
  ...
  renderGuidesList() {
    ...
    this.guides.forEach(guide => {
      ...
      tr.innerHTML = `
        ...
        <td>${guide.date || ""}</td>
        <td>${guide.author || "AeroVape Education"}</td>
  ...
  async saveGuide() {
    ...
    const authorVal = document.getElementById("guide-author").value.trim() || "AeroVape Education";
  ...
  editGuide(guideId) {
    ...
    document.getElementById("guide-author").value = guide.author || "AeroVape Education";
  ...
  async updateOrderStatus(orderId, newStatus) {
    ...
    localStorage.setItem("aero_vape_orders", JSON.stringify(this.orders));
  ...
  async clearOrders() {
    ...
    localStorage.removeItem("aero_vape_orders");
    this.orders = [];
    this.renderOrders();
  }
...
We're so glad you chose AeroVape — thank you for your order! Since automatic email receipts can sometimes be delayed or blocked by spam filters, we're personally sending this confirmation so you have everything you need to complete your payment and get your order on the way.
...
Thank you so much for supporting AeroVape — we truly appreciate it!
For any questions, feel free to reply to this email or reach us at ${this.config.settings.contactEmail || "orders@aerovape.co"}.
...
AeroVape Team`;
  ...
  bindEvents() {
    ...
    if (logoutBtn) logoutBtn.addEventListener("click", () => {
      sessionStorage.removeItem("aero_vape_admin_authenticated");
      location.reload();
    });
  }

  async createTestOrder() {
    const confirmText = "Are you sure you want to generate a random test order? This will sync to Firebase and send emails to orders@aerovape.co.";
    ...
    try {
      ...
      const email = "orders@aerovape.co"; // Fixed test email
        ...
        try {
          ...
          await emailjs.send(
            ...
            settings.emailjsOrderTemplateId.trim(),
            {
              ...
              ref_code: refCode,
              customer_name: name,
              customer_email: email, // orders@aerovape.co
              reply_to: email,
      ...
      alert(`Test Order ${orderId} created successfully!\n\nAll notifications (EmailJS & FormSubmit) have been dispatched to orders@aerovape.co.`);
