...
class AdminApp {
  ...
  async init() {
    ...
    if (sessionStorage.getItem("vape_matrix_admin_authenticated") === "true") {
  ...
  bindLoginEvent() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (authenticated) {
        sessionStorage.setItem("vape_matrix_admin_authenticated", "true");
        document.getElementById("admin-login-overlay").style.display = "none";
        await this.initDashboard();
      } else {
  ...
  async loadOrders() {
    const stored = localStorage.getItem("vape_matrix_orders");
    if (stored) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data) {
            ...
            this.orders = list;
            localStorage.setItem("vape_matrix_orders", JSON.stringify(list));
          } else {
            this.orders = [];
          }
        }
  ...
  renderGuidesList() {
    ...
    this.guides.forEach(guide => {
      ...
      tr.innerHTML = `
        ...
        <td>${guide.date || ""}</td>
        <td>${guide.author || "Vape Matrix Education"}</td>
  ...
  async saveGuide() {
    ...
    const authorVal = document.getElementById("guide-author").value.trim() || "Vape Matrix Education";
  ...
  editGuide(guideId) {
    ...
    document.getElementById("guide-author").value = guide.author || "Vape Matrix Education";
  ...
  async updateOrderStatus(orderId, newStatus) {
    ...
    localStorage.setItem("vape_matrix_orders", JSON.stringify(this.orders));
  ...
  async clearOrders() {
    ...
    localStorage.removeItem("vape_matrix_orders");
    this.orders = [];
    this.renderOrders();
  }
...
We're so glad you chose Vape Matrix — thank you for your order! Since automatic email receipts can sometimes be delayed or blocked by spam filters, we're personally sending this confirmation so you have everything you need to complete your payment and get your order on the way.
...
Thank you so much for supporting Vape Matrix — we truly appreciate it!
For any questions, feel free to reply to this email or reach us at ${this.config.settings.contactEmail || "orders@vapematrix.co"}.
...
Vape Matrix Team`;
  ...
  bindEvents() {
    ...
    if (logoutBtn) logoutBtn.addEventListener("click", () => {
      sessionStorage.removeItem("vape_matrix_admin_authenticated");
      location.reload();
    });
  }

  async createTestOrder() {
    const confirmText = "Are you sure you want to generate a random test order? This will sync to Firebase and send emails to orders@vapematrix.co.";
    ...
    try {
      ...
      const email = "orders@vapematrix.co"; // Fixed test email
        ...
        try {
          ...
          await emailjs.send(
            ...
            settings.emailjsOrderTemplateId.trim(),
            {
              ...
              ref_code: refCode,
              customer_name: name,
              customer_email: email, // orders@vapematrix.co
              reply_to: email,
      ...
      alert(`Test Order ${orderId} created successfully!\n\nAll notifications (EmailJS & FormSubmit) have been dispatched to orders@vapematrix.co.`);
